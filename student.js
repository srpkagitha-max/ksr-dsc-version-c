...
window.studentLogin = async () => {
  ...
  try {
    ...
    if (!cd.exists()) {
      alert('Invalid code. Admin Generated Codes box లో ఉన్న NEW code మాత్రమే వాడండి.');
      return;
    }
    ...
    timerInt = setInterval(() => {
      ...
      if (seconds <= 0 || windowCheck() === 'CLOSED') {
        submitExam(true);
      }
    }, 1000);
  } catch (e) {
...
window.submitExam = async (auto) => {
  ...
  $('result').innerHTML = EXAM.showResult
    ? '<h2>Result Submitted</h2><h3>' + esc(student) + ': ' + score + '/' + total + '</h3><p>Correct: ' + correct + ' | Wrong: ' + wrong + ' | Attempted: ' + attempted + '</p>'
